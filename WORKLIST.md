...
**★ 完整 RPG 系統(上游無,本 fork 從零補完成可通關遊戲)**
- **建角開場**(U1 招牌:性別/種族/職業 + 30 點屬性分配,種族給屬性修正、職業給起始裝備法術)。
- **商店**(6 類:武器/防具/食物/魔法/酒館/船塢)+ 物品欄 + 裝備。
- **戰鬥深度**:傷害接武器/防具/屬性、擊殺得 XP→升級回滿+成長、死亡回城堡復活。
- **國王試煉**(城堡 T 領任務領獎)、**魔法**(地牢 C:飛彈/致死/升降梯/祈禱)。
- **角色屬性表**(Z)、**載具**(木筏渡水)、**勝利結局**(4 試煉→終結蒙登,上游無 win condition)。
- 完整弧線:探索→城鎮買裝備法術→地牢戰鬥升級→4 國王試煉→終結蒙登→勝利。規劃見 `docs/plan-town-combat.md`。

**遊戲系統 / 操作補完**
- **存檔系統**(`SaveGame`:玩家狀態+物品欄+設定序列化、F10 離開 autosave、F5 手動、啟動載入)。
...
- [x] **Phase C — 城堡/國王/法術/酒館/Ztats**:城堡 T 國王任務(討伐 N→金幣+力量);地牢 C 施法(飛彈/致死/升降梯/祈禱);Z 角色屬性表;酒館(商店第5類)買線索。全截圖驗證。
- [x] **Phase D — 載具 + 勝利結局**:船塢(商店第6類)買木筏 → 世界地圖水域可通行;完成 4 國王試煉 → 終結蒙登 → 勝利畫面(上游無 win condition)。截圖驗證。
- [x] **Phase E — 建角開場**:`CharCreation.h`(5 種族屬性修正 / 4 職業起始裝備法術 / 30 點六屬性分配);新遊戲進主迴圈前跑建角 modal,套到 Player(屬性/HP/裝備/法術)。i18n cc.* 全繁中;game_tester t00 步驟驅動 Enter 進場。截圖驗證(進場生命=120 證實套用生效)。
- [x] **地牢標籤糊字修正**:層數/朝向從 320 target 12px cjk() 移到 640 層 16px cjkUi(),複雜漢字「層」不再糊成錯字。

> **可通關狀態(2026-06-27)**:建角 → 探索(7 平台美術+音樂)→ 城鎮(6 類商店+酒館)→ 城堡(國王試煉)→
> 第一人稱地牢(戰鬥+魔法+升降梯)→ 載具渡水 → 完成 4 試煉 → 終結蒙登 → 勝利。
