...
| E5 | **Atari** | 1983 ATR(已抽檔)| 🟡 **charset 已定位**:overworld 圖形程式 `OUTMOVE.bin`(Atari binary load 格式),反組譯找到 **CHBAS($02F4)=$64 → charset @$6400**,ANTIC 4 多彩字元,~38 個重定義字模($6400-$6497 + $6500-$6597)。但**tile 為多字元組合 + 顏色情境相關**,完整抽取需深挖 map-draw 6502 碼(SET1-5.bin 非 charset,是地圖/資料);低投報暫緩。screenshot oracle 僅 atari800-04 一張 overworld(~4 tile 入鏡)覆蓋不足。 | 🟡 格式定位,完整抽取暫緩 |
