...
| E5 | **Atari** | 1983 ATR(已抽檔)| 🟡 **進行中**(詳見 `docs/re/atari-tileset-re.md`):反組譯 `OUTMOVE.bin`(Atari binary load 格式)→ `CHBAS=$6400` 是**文字字型**;用實機圖 ANTIC4/5 邏輯像素結構比對證明 **overworld tile 不在 $6400 charset**。已 rip **4 個 authentic tile**(water/grass/forest/player,`rip_atari_tiles.py`)+ 反推 overworld palette(黑/橄欖綠/藍/灰)。**下一步**:tile 圖在第二套 charset(SET1-5/DLI 換 CHBAS)或軟體 bitmap/PMG → 反組譯 map-draw 反追指標(同 IIgs `$4c00` 追法),或同源 Apple II 截圖補。**不放棄(完整性原則)**。 | 🟡 4 tile authentic,完整抽取進行中 |
