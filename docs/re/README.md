...
## Step 7 — 鏈載入器 + 常駐引擎(架構牆,已驗證)

- **鏈載入器** `[CODE]`:`ULTIMA` $8024 設好 DOS 載入向量後,$8049 內嵌 ATASCII `D:INITLOAD`
  → 載下一檔;逐檔鏈載組出常駐引擎。
- 各 init/move 檔多載到 `$8000` 暫存區;**無檔直接載 `$3000-$69FF`** → 常駐引擎疑似從 $8000
  **重定位**到 $3000($6A00 那串 ASL/ROL 位址運算像重定位器)。
- ⇒ **怪物移動的通用回合迴圈在常駐引擎($3000),由鏈載入 + 重定位組成,且 overlay 無獨立入口**
  (入口在常駐調度表)。逐行隔離 = 反組譯整個常駐引擎,**多日工程**。
- 已把所有「`[CODE]` 架構事實 + `[BEHAV]` 行為」整合進 `docs/ultima1-original-ai.md`,
  足以**忠於原版實作**;確切 6502 routine 待續(下方續追路徑)。

### 續追路徑(若要 `[CODE]` 逐行確認怪物移動)
1. 模擬鏈載入(ULTIMA→INITLOAD→…)組出常駐引擎在 $3000 的最終映像(含重定位)。
2. 反組譯常駐引擎,從 `$3019` 向量呼叫端找地牢回合 handler。
3. 在回合迴圈找「CMP 怪座標 vs 玩家座標 → INC/DEC 怪座標」段落,內容對齊驗證。

### 狀態:架構全解明;AI 行為已足以實作,確切 6502 routine 待續(非受阻,工程量大)
