...
### 版面(橫向 landscape,雙拇指)— 已定案(2026-06-28 與 L.CY 討論)

**決策**:① 系統/F 鍵 = **常駐頂列小圖示**(7 顆,永遠可見、零額外點擊);
② 中間動作鈕 = **依場景自動顯示**(一次只露 3–4 顆,不誤觸)。

```
┌──────────────────────────────────────────────────┐
│ [?說明][💾存][⚙設][♪樂][🔊效][▣風格][⏻離]         │ ← 常駐頂列(F1/F5/F6/M/F9/PageDown/F10)
│                                                   │
│        ↑                              (A)攻擊      │
│     ←  ·  →        遊戲畫面            (E)(Z) …    │ ← 情境動作鈕(隨場景變)
│        ↓                              (B返回/ESC) │
...
| 觸控元件 | 合成鍵 | 說明 |
|---|---|---|
| D-pad ↑↓←→ | 方向鍵 | 移動・轉向 / 選單上下左右 |
| **A**(主鈕)| `A`(選單時 `Enter`)| 攻擊・射擊 / 選單確定 |
| **B**(返回)| `ESC` | 取消・返回(離開鐵則;選單時 = 取消)|
| 情境動作鈕 | `E/K/C/B/T/Z/L` | **依場景自動顯示**(見下表)|
| 常駐頂列 7 顆 | `F1/F5/F6/M/F9/PageDown/F10` | 說明・存檔・設定・音樂・音效・畫面風格・離開 |

**情境動作鈕(依 `getCurrentScreen()` + `isInCastle()` 動態顯示)**:
| 場景 | 顯示的動作鈕 |
|---|---|
| 世界地圖 | A 攻擊 · E 進入 · Z 屬性(+ L 升空,有太空梭時)|
| 地牢 | A 攻擊 · K 攀爬 · C 施法 · Z 屬性 |
| 城鎮 | E 離開 · B 商店 · Z 屬性 |
| 城堡 | E 離開(救公主)· T 國王 · Z 屬性 |
| 星海 | A 射擊(D-pad 左右移、B 撤離)|

> 選單模式(shop/king/cast/ztats/settings/建角/離開框/勝利)只需 **D-pad + A(=Enter) + B(=ESC)**,
> 頂列照常可用 → 一套觸控層通吃遊戲與選單。頂列圖示可用簡單字符/2 字標籤(自帶字型已含)。
