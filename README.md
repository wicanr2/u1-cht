...
- **磁碟機讀取音效**:進出城鎮/城堡/地牢時,可播老 5.25" 磁碟機的「起轉 → 尋道」讀取聲(懷舊效果)。`F6` 設定裡切換音源——**關(預設)/ MAME / Applefritter**;音源用的是真實磁碟機錄音(MAME `samples/floppy`,CC0),不是合成音。忠於原版預設關,懷舊玩家自行開啟。
...
| `F5` / `F9` | 手動存檔 / 音效開關 |
| `F6` | 設定選單:時間流速 / 生怪率 / 食物消耗 / 野外怪物追蹤 / 磁碟機音效(關·MAME·Applefritter) |
